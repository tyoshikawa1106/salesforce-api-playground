...
| Recycle Bin 対象 object / item | `lib/salesforce/recycle-bin.ts`, `lib/salesforce/request-payloads.ts` | `/api/recycle-bin`, `/api/recycle-bin/undelete` |
...
この境界に従って、HTTP 入口、共通検証、Salesforce データ操作を分けます。
...
| `app/api/recycle-bin-routes.test.ts` | Recycle Bin Route の service 呼び出し、Origin / Referer と復元 payload 検証 |
...
| `services/salesforce/recycle-bin.test.ts` | Recycle Bin の `scanAll` query と `undelete` 呼び出し、混在 item の正規化 |
