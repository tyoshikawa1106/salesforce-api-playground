...
このページは、リポジトリの主要ディレクトリ、配置判断、ファイルごとの用途を確認するための入口です。

対象は Git 管理されているファイルです。`.next`、`coverage`、`node_modules`、`.env.local` などの生成物や秘密情報を含み得るローカルファイルは対象外です。

ファイル一覧は陳腐化しやすいため、各ファイルの内部仕様までは書かず、「どこに何があるか」と「変更時に触る入口がどれか」を確認できる説明に留めます。
...
| `docs` | 開発者向け一次情報 | 設計、API、セットアップ、運用、デプロイ、セキュリティ、開発ナレッジ | GitHub Releases として管理すべき正式なリリースノート |
...
| `docs/architecture` | システム構成、Salesforce レコードモデル、UI flow、ディレクトリなどの設計情報 |
| `docs/api` | API Routes と Salesforce API 連携の仕様 |
| `docs/security` | OAuth flow、秘密情報などセキュリティ関連の設計 |
| `docs/setup` | ローカル開発、Salesforce 設定、手動確認 |
| `docs/deployment` | Heroku デプロイと運用確認 |
| `docs/operations` | GitHub 運用、CI、困ったとき |
