...
このリポジトリでは、画面の URL 入口を `app/**/page.tsx`、API を `app/api/**/route.ts` に置き、Next.js App Router の構成で実装しています。
...
このリポジトリでは `package.json` の `engines` で `node: 24.x` と `npm: >=11` を指定しています。
