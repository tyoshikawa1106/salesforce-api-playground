...
`app/page.tsx` はトップページの入口。環境ラベルを取得し、画面本体である `components/Playground.tsx` を表示する。
...
一方で `app/page.tsx` は `"use client"` を持たず、環境ラベルを取得して Client Component へ渡す入口として動く。
...
トップページの入口は `app/page.tsx`。ただし表示の本体は `Playground.tsx` にあるため、UI の変更は `components` 側を読む。
