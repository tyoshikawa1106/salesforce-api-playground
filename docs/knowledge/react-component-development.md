...
| UI 部品 | `shell/GlobalHeader.tsx`, `records/RecordListTable.tsx`, `shell/Modal.tsx` | JSX と SLDS class を中心に見る |
| hooks | `hooks/usePlaygroundData.ts`, `records/useRecordMutations.ts`, `shell/useGlobalSearch.ts` | state、effect、handler の流れを見る |
| API helper | `utils/api.ts` | fetch、response、error handling を見る |
| 純粋なロジック | `records/record-list-state.ts`, `records/record-actions.ts`, `records/mutations.ts`, `utils/formatting.ts` | 入力と戻り値に注目する |
| 型定義 | `utils/types.ts`, `records/record-list-types.ts` | データ構造と props の形を見る |
...
components/Playground.tsx
  ↓
components/playground/shell/GlobalHeader.tsx
  ↓
components/playground/shell/GlobalHeaderActions.tsx
  ↓
components/playground/shell/useGlobalHeaderMenus.ts
```
...
components/playground/PlaygroundWorkspace.tsx
  ↓
components/playground/records/RecordWorkspacePanels.tsx
  ↓
components/playground/records/RecordListPanel.tsx
  ↓
components/playground/records/RecordListTable.tsx
```
...
components/Playground.tsx
  ↓
components/playground/records/useRecordMutations.ts
  ↓
components/playground/records/mutations.ts
  ↓
components/playground/records/record-actions.ts
  ↓
...
配置判断は [React コンポーネント配置の考え方](react-component-organization.md) も参照する。
