...
このリポジトリでは、Release notes を正式なリリースノートとして扱います。`CHANGELOG.md` は作成しません。
...
Dependabot PR は `main` に向けて作成されます。npm 依存関係更新には `type:maintenance`、GitHub Actions 更新には `area:github` と `type:maintenance` の label が付く設定です。
