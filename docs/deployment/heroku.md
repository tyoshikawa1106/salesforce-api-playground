...
| 事象 | 確認すること | 対処 / 判断 |
| --- | --- | --- |
| Heroku release が作成されない | GitHub `main` への merge が完了しているか、GitHub Actions が pass しているか、Heroku の自動デプロイ設定が有効か | Heroku 自動デプロイ設定の画面手順は未確認 |
| Heroku build が失敗する | `npm ci`、`npm run build` が CI / ローカルで通るか、Node.js / npm engines に合っているか | build log の詳細取得手順は未確認 |
...
- Salesforce 外部クライアントアプリケーションの最新 UI に基づく callback URL 設定画面の詳細手順は未確認。
