...
このリポジトリでは、Staging app は GitHub `stage` から、Production app は GitHub `main` から自動デプロイされる運用です。通常の開発 PR は作業ブランチから `stage` に向け、Staging 確認後に `stage` から `main` へ PR を作成して本番反映します。PR はユーザーが merge し、merge 後に GitHub Actions、Heroku release、dyno 起動状態を確認します。
...
- GitHub `stage` が本番前確認用ブランチ、`main` が本番デプロイ用ブランチであること。
- Staging app の GitHub 自動デプロイが `stage` に設定済みであること。
...
Heroku release 履歴では `Deploy <commit>` 形式の release が確認できています。デプロイ確認では、release の説明に記録された commit hash と GitHub `stage` または `main` の commit hash の対応を確認します。
...
| --- | --- | --- | --- |
| `stage` | Staging app | 本番前の統合確認 | `stage` push workflow、Staging release、Staging dyno |
| `main` | Production app | 本番反映 | `main` push workflow、Production release、Production dyno |

通常開発の流れ:

1. `codex/...` などの作業ブランチから `stage` へ draft PR を作成する。
2. GitHub Actions が pass したら ready for review にする。
3. ユーザーが `stage` へ merge する。
4. Staging app の release、dyno、必要な手動確認を行う。
5. Staging 確認後、`stage` から `main` へ PR を作成する。
...
1. 作業ブランチから `stage` への PR で GitHub Actions が pass していることを確認する。
...
1. `stage` の最新 commit を確認する。
2. GitHub Actions の `stage` push workflow が pass していることを確認する。
3. Staging app に Heroku release が作成され、release の commit hash が `stage` の merge commit と対応していることを確認する。
...
1. `stage` から `main` への PR で GitHub Actions が pass していることを確認する。
...
| 事象 | 確認すること | 対処 / 判断 |
| --- | --- | --- |
| Staging release が作成されない | GitHub `stage` への merge が完了しているか、GitHub Actions が pass しているか、Staging app の自動デプロイ設定が `stage` か | release 履歴で commit hash との対応を見る |
...
- Heroku の rollback 操作が GitHub `stage` または `main` との差分を発生させる点を運用上許容するか。
- rollback 後に `stage` または `main` へ修正 PR を作成する必要があるか。
...
rollback 後は GitHub `stage` または `main` と Heroku runtime の内容が一時的にずれる可能性があります。原因修正は PR で反映してください。
...
- Heroku rollback は Codex 作業で実行しない。必要な場合も、対象 release と GitHub `stage` または `main` との差分を確認してから運用者が判断する。
