...
Heroku 自動デプロイの画面設定手順は未確認です。現状の運用では、Heroku release の説明に GitHub merge commit と対応する commit hash が記録されることを確認しています。

直近の確認済み情報:

| 確認項目 | 確認済み内容 |
| --- | --- |
| PR merge 後の release | PR #44 merge 後に `v35 Deploy 446788ba` を確認済み |
| merge commit との対応 | merge commit `446788b` と release の commit hash が対応することを確認済み |
| dyno 起動状態 | `web.1 up` を確認済み |
...
| Node.js | `package.json` の `engines.node` により `>=20 <23` |
...
| `SALESFORCE_API_VERSION` | 任意 | `v60.0` | 未設定時は実装上 `v60.0`。`jsforce.Connection` には先頭 `v` を除いて渡す |
...
2. CI は `.github/workflows/ci.yml` に基づき、Node.js 20 で以下を実行する。
...
`heroku builds` による build 詳細確認は、現在のローカル Heroku CLI ではコマンド未提供のため未確認です。
...
| 事象 | 確認すること | 対処 / 判断 |
| --- | --- | --- |
| Heroku release が作成されない | GitHub `main` への merge が完了しているか、GitHub Actions が pass しているか、Heroku の自動デプロイ設定が有効か | release 履歴で commit hash との対応を見る。Heroku 自動デプロイ設定の画面手順は未確認 |
| Heroku build が失敗する | `npm ci`、`npm run build` が CI / ローカルで通るか、Node.js / npm engines に合っているか | build log の詳細取得手順は、現在のローカル Heroku CLI では `builds` コマンドが未提供のため未確認 |
...
Heroku release は作成されていることを確認していますが、このリポジトリでの実ロールバック手順は未確認です。

未確認のため、ロールバックが必要な場合は以下を事前に確認してから実行してください。
...
一般的な Heroku CLI には rollback 系コマンドがありますが、このリポジトリの運用として実行確認していないため、ここでは確定手順として記載しません。
...
- Heroku 自動デプロイの実画面設定は、変更時に運用者が確認して追記する。

## TODO / 未確認

- Heroku の GitHub 自動デプロイ設定画面の確認手順は未確認。
- Heroku build log の取得手順は、現在のローカル Heroku CLI では `builds` コマンドが未提供のため未確認。
- Pipeline の利用有無と確認手順は未確認。
- このリポジトリ運用での Heroku rollback 実行手順は未確認。
