...
このリポジトリでは、GitHub `main` への merge 後に Heroku へ自動デプロイされる運用を基本にします。通常の開発 PR は作業ブランチから `main` に向けます。PR はユーザーが merge し、merge 後に GitHub Actions、Heroku release、dyno 起動状態を確認します。
...
- Heroku app の GitHub 自動デプロイが `main` に設定済みであること。
...
Staging app の作成有無、GitHub auto deploy の対象ブランチ、Config Vars の実値は Heroku Dashboard で確認します。GitHub Flow では `main` への merge をデプロイの起点にします。Heroku app 名、Git URL、Web URL、Owner などの実運用値はこのドキュメントに記載しません。
...
| `main` | Heroku app | デプロイ可能な安定版 | `main` push workflow、Heroku release、dyno |
...
5. Heroku app の release、dyno、必要な手動確認を行う。
...
3. Heroku app に release が作成され、release の commit hash が `main` の merge commit と対応していることを確認する。
4. Heroku app の dyno が `web: npm run start` で `up` になっていることを確認する。
5. Runtime で `GET /api/session` が秘密情報を返さず、未接続時に `connected: false` 相当のレスポンスになることを確認する。
...
| 事象 | 確認すること | 対処 / 判断 |
| --- | --- | --- |
| Heroku release が作成されない | GitHub `main` への merge が完了しているか、GitHub Actions が pass しているか、Heroku app の自動デプロイ設定が `main` か | release 履歴で commit hash との対応を見る |
