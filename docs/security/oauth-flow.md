...
画面項目ごとの設定、Integration ユーザー、権限セット、疎通確認、詰まりやすいエラーは [Integration ユーザー設定](../setup/salesforce-integration-client-credentials.md) を参照してください。
...
2026-06-06 時点の Salesforce Help では、External Client Apps の OAuth 設定で `OAuth Settings`、`Callback URL`、OAuth scope、Authorization Code / Client Credentials 関連の flow 設定を扱います。画面ラベルは Salesforce のリリースや組織設定で変わる可能性があるため、このドキュメントでは画面手順の完全な写しではなく、現行実装が必要とする OAuth 要件を設定表として管理します。

refresh token の失効条件や有効期限は Salesforce 組織の OAuth policy、管理者設定、revoke 操作に依存します。現行実装では refresh に失敗した場合、session Cookie と state Cookie を削除して `401` を返します。
