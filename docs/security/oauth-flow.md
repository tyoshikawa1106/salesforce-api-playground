...
`/api/integration/*` はユーザーの Connect 操作を使わず、Salesforce 外部クライアントアプリケーションの Client Credentials Flow で access token を取得します。
...
`/api/integration/*` は Cookie セッションを使わないため、呼び出し元検証として `x-integration-api-key` ヘッダーを必須にしています。ヘッダー値が `INTEGRATION_API_KEY` と一致しない場合は Salesforce token endpoint を呼ばずに `401` を返します。
