...
| `app/api` | HTTP API のエントリポイント | OAuth、session、Account / Contact、活動、選択リスト値、検索、Integration API の Route Handler | Salesforce CRUD の実体、Cookie 暗号化や入力検証の重複実装 |
...
| `services/salesforce` | Salesforce データ操作層 | `jsforce.Connection` 作成、access token refresh 後の再試行、Account / Contact の SOQL と CRUD、活動、選択リスト値、Recycle Bin | Cookie session の暗号化、OAuth URL 組み立て、request payload 検証 |
| `docs` | 開発者向け一次情報 | リポジトリ構成、API、画面、開発・運用、デプロイ、開発ナレッジ | GitHub Releases として管理すべき正式なリリースノート |
