...
決定事項は、背景、選択肢、決定内容、影響範囲、見直し条件が後から分かるように記録します。実装から確認できない内容は断定せず、必要なら具体的な確認内容を Issue または PR コメントに記録してください。

## 前提条件

- 重要な設計変更、依存関係追加、運用変更、セキュリティ方針変更があること
- 既存の `AGENTS.md` と矛盾しないこと
- 矛盾する場合は既存ルールを優先し、必要なら具体的な確認内容を Issue または PR コメントに記録すること
...
以下は新しい ADR を作成するときに本文へコピーし、角括弧のプレースホルダーを置き換えて使います。実タスクの未完了項目ではありません。
