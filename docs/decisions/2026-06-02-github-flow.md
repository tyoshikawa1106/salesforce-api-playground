...
旧 Git Flow の `develop`、`release/*`、`hotfix/*` は残っているが、新規開発の base として使うと、Heroku Staging への自動デプロイ起点である `main` と差分管理が分散する。
...
- `main` へ直接コミットしない。
- `develop`、`release/*`、`hotfix/*` は旧運用のブランチとして凍結扱いにし、新規開発の base には使わない。
...
- 作業開始時は `main` を最新化してから `codex/...` ブランチを作成する。
