...
Project 自動追加の実効状態は、workflow 全体の conclusion だけで判断しません。secret 未設定時も workflow は success になり、Project 追加 step だけが skipped になります。以下のように対象 run の job / step 結果を確認します。
...
| --- | --- | --- |
| `issues.opened` | `Add issue to project` step が success | `Add issue to project` step が skipped になり、`Skip project assignment` step が success |
| `pull_request.opened` | `Add pull request to project` step が success | `Add pull request to project` step が skipped になり、`Skip project assignment` step が success |
| `issues.closed` | `Mark issue as Done` step が success | `Mark issue as Done` step が skipped になり、`Skip project status update` step が success |
| `pull_request.closed` | merge 済み PR で `Mark pull request as Done` step が success | `Mark pull request as Done` step が skipped になり、`Skip project status update` step が success |

Project item の追加結果は、以下で対象 URL を検索して確認します。

```bash
gh project item-list 1 --owner tyoshikawa1106 --format json --limit 1000 --jq '.items[] | select(.content.url == "https://github.com/tyoshikawa1106/salesforce-api-playground/issues/<Issue番号>" or .content.url == "https://github.com/tyoshikawa1106/salesforce-api-playground/pull/<PR番号>") | {id,title,status,type:.content.type,url:.content.url}'
```

2026-06-02 の確認では、Issue #242 作成時の run `26823730486`、PR #246 作成時の run `26846590632`、PR #240 作成時の run `26823240121`、PR #235 作成時の run `26821262345` はいずれも workflow conclusion は success でしたが、Project 追加 step は skipped、skip 理由 step は success でした。`gh secret list` でも `GH_PROJECT_TOKEN` は確認できなかったため、少なくともこの確認時点では Project 自動追加は実効しておらず、Project 追加は手動対応が必要な状態でした。
