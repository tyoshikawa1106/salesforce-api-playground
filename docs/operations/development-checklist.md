...
このドキュメントは、作業ブランチで変更を行う前後に、実装、ドキュメント、確認コマンド、PR 本文で見る項目をまとめます。

詳細な GitHub Flow、Issue / PR / milestone / Project の運用は [GitHub 運用](github.md)、確認コマンドの選び方は [CI](ci.md)、ドキュメント更新先の判断は [ドキュメント保守](documentation-maintenance.md) を参照してください。
...
| UI / SLDS | `components/Playground.tsx`, `components/playground`, `app/globals.css` | [Playground UI 操作フロー棚卸し](../architecture/playground-ui-flows.md), [Salesforce 手動確認](../setup/salesforce-manual-verification.md) |
...
| 環境変数 / 起動方式 | `.env.example`, `package.json`, `Procfile`, `app.json` | [ローカル開発](../setup/local-development.md), [Heroku デプロイ](../deployment/heroku.md), [秘密情報の扱い](../security/secret-handling.md) |
| GitHub Actions / templates | `.github/workflows`, `.github/ISSUE_TEMPLATE`, `.github/pull_request_template.md` | [GitHub 運用](github.md), [CI](ci.md), [トラブルシューティング](troubleshooting.md) |
...
| docs のみ | `README.md`, `docs`, `.github/*template*` | [ドキュメント保守](documentation-maintenance.md), [CI](ci.md), [秘密情報の扱い](../security/secret-handling.md) |
...
| 管理情報 | milestone、Project `Salesforce API Playground`、label を設定する。設定できない場合は理由を書く |
| Draft | 通常開発 PR は Draft で作成し、required checks pass 後に ready for review にする |
| 確認表 | `.github/pull_request_template.md` の動作確認表に実行結果を記載する |
| 未実行確認 | レビュー判断に関係する未実行確認だけ、理由と一緒に記載する |
