...
GitHub Actions CI は Pull Request と `stage` / `main` ブランチへの push で実行されます。Node.js 24 で依存関係を `npm ci` でインストールしたあと、以下を順番に確認します。
...
| 事象 | 確認すること | 対処 |
| --- | --- | --- |
| GitHub `stage` merge 後に Staging release が作成されない | PR merge 後の `stage` push workflow が pass しているか、Staging app の GitHub 自動デプロイ設定が `stage` になっているか | Heroku release 履歴で commit hash との対応を見る |
...
| ロールバックしたい | 戻す release 番号、commit hash、GitHub `stage` または `main` との差分を確認する | Codex 作業では rollback を実行しない |
