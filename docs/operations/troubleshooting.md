...
| API route の呼び出しが多い | 利用者数、実行元、Salesforce API 使用量を確認する | アプリ独自の rate limit は未実装 |
...
- Salesforce 組織固有の validation rule、権限、参照整合性エラーが発生した場合は [共通事項](../api/common.md) と Salesforce の `details` を確認する。
