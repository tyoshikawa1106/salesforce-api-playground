...
| `docs/development` | 実装時の実務チェックリストと開発運用ルールを置く。 |
| `docs/setup` | ローカル環境設定、CLI、CI、Salesforce 設定を置く。 |
...
| `docs/development` | 実装時に参照する実務向けチェックリストと開発運用ルール |
| `docs/setup` | ローカル環境設定、CLI、CI、Salesforce 設定 |
...
| `.github/workflows` | CI などの GitHub Actions |
