...
5. 開発サーバーを起動する。
    ...
    ローカル開発では、通常 `http://localhost:3000` を使います。Next.js が別 port の既存 dev server を検出した場合は、前回の起動プロセスが残っていないか確認し、不要な server を停止してから起動し直します。
...
7. 作業が終わったら、`npm run dev` を実行している terminal で `Ctrl+C` を押して dev server を停止する。
...
coverage 対象は `app/api/**/*.ts`、`lib/**/*.ts`、`services/salesforce/**/*.ts` です。`*.test.ts` と `app/api/test-helpers.ts` は対象から除外します。
