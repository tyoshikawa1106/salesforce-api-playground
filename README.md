...
`npm run slds:lint` は SLDS の style / component lint を実行します。`npm run slds:lint:fix` は linter が自動修正できる指摘を反映します。自動修正後は差分を確認し、必要に応じて `npm run lint` / `npm run typecheck` / `npm run build` を実行してください。

このプロジェクトでは Next.js 14 の ESLint 8 系と SLDS Linter が内部で使う ESLint 9 系を共存させるため、`slds-linter.eslint.config.mjs` を指定して実行します。

## テストカバレッジ

Vitest の V8 coverage provider で、API route と `lib` 配下のテストカバレッジを確認できます。

```bash
npm run test:coverage
```

実行するとターミナルにファイル別のカバレッジと summary が表示されます。HTML レポートも `coverage/index.html` に生成されます。
