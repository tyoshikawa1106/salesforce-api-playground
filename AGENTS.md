...
- `gh issue create`、`gh pr create`、`gh pr edit` などで複数行 Markdown 本文を渡す場合は、shell 引数内の `\n` に頼らず、一時ファイルなどに実改行で本文を書いて `--body-file` で渡す。作成・更新後は `gh issue view` / `gh pr view` で本文を確認し、GitHub 上で `\n` が文字列として表示されていないことを確認する。
