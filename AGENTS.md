...
- 作業開始時はローカルの `codex/...` ブランチも確認し、完了済みだが未マージの作業が残っていないか把握する。
...
- URL の query / path に外部入力や Salesforce record id を入れる場合は、`URLSearchParams` または `encodeURIComponent` でエンコードする。OAuth / Salesforce endpoint URL は検証済みの HTTPS origin を使い、詳細は [共通事項](docs/api/common.md) と [ローカル開発](docs/setup/local-development.md) を参照する。
...
PR 作成前に `.github/pull_request_template.md` を確認し、`Issue`、`変更内容`、`確認結果`、`レビュー観点` などのセクション名と順序を維持する。背景や補足説明を追加する場合も、テンプレートの該当セクション内に収める。
